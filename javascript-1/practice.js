...
let total = myNumbers.reduce((t, v) => t + v);
...
let myNumbersIndex = [];
myNumbers.forEach((v, i) => myNumbersIndex.push(i));
...
let forTheLoveOfGeorge = notGeorge.map(v => 'George');
...
let enemies = people.filter(v => !(v.friend));
...
let totallyAwesome = people.reduce((t, v) => {
	return t + v.awesomeLevel;
}, 0);
